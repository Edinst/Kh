...
module.exports = {
  config: {
    ...
    longDescription: { en: "Text to image" },
    category: "image",
    guide: {
  ...
  onStart: async function ({ api, event, args, message, usersData }) {
    ...
    message.reply("✅| Creating your Imagination...", async (err, info) => {
      ...
      try {
        const response = await axios.get(`https://milanbhandari.imageapi.repl.co/milanisgodig?prompt=${encodeURIComponent(prompt)}&model=${model}`);
        api.setMessageReaction("✅", event.messageID, () => {}, true);
...
function reduceGold(fbid) {
  ...
  if (userBalance) {
    if (userBalance.gold >= 1200) {
      userBalance.gold -= 1200;
